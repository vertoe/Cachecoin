...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // cachecoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                              ...
                              enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 CACH if made of 50 CACH inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
                        ...
                        bool* pfMissingInputs)
{
    ...
    // cachecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// cachecoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // cachecoin: subsidy is cut in half every 64x multiply of difficulty
...
// cachecoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// cachecoin: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
 ...
 /*            if(fTestNet || (pindexLast->nHeight + 1 > 15000))
    ...
    // cachecoin: target change every block
    // cachecoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal cachecoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // cachecoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // cachecoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // cachecoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // cachecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // cachecoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    if (!txdb.WriteBlockIndex(CDiskBlockIndex(pindex)))
        return error("Connect() : WriteBlockIndex for pindex failed");

    // cachecoin: fees are not collected by miners as in cachecoin
    // cachecoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// cachecoin: total coin age spent in transaction, in the unit of coin-days.
...
// cachecoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // cachecoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // cachecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // cachecoin: record proof-of-stake hash value
    ...
    // cachecoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // cachecoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // cachecoin: coinbase output should be empty if proof-of-stake block
    ...
    // cachecoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // cachecoin: check that the block satisfies synchronized checkpoint
    ...
    // cachecoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // cachecoin: check proof-of-stake
    ...
    // cachecoin: verify hash target and signature of coinstake tx
    ...
    // cachecoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // cachecoin: getblocks may not obtain the ancestor block rejected
    ...
    // cachecoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // cachecoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // cachecoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    if (GetBoolArg("-testsafemode"))
        strRPC = "test";
/*
    // cachecoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
...
*/
    ...
    // cachecoin: should not enter safe mode for longer invalid chain
    // cachecoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // cachecoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // cachecoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // cachecoin: record my external IP reported by peer
        ...
        // cachecoin: relay sync-checkpoint
        {
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // cachecoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // cachecoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // cachecoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // cachecoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("CachecoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("CachecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CachecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("CachecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadCachecoinMiner(void* parg);

static bool fGenerateCachecoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void CachecoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateCachecoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateCachecoins) && !fProofOfStake)
                return;
        }
        ...
        if (fProofOfStake)
        {
            // cachecoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running CachecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadCachecoinMiner(void* parg)
{
    ...
    try
    {
        ...
        CachecoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadCachecoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadCachecoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadCachecoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateCachecoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateCachecoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateCachecoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d CachecoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadCachecoinMiner, pwallet))
                printf("Error: NewThread(ThreadCachecoinMiner) failed\n");
            Sleep(10);
        }
    }
}
