...
#ifndef CACHECOIN_MAIN_H
#define CACHECOIN_MAIN_H
...
public:
    ...
    {
        // cachecoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    /** Amount of cachecoins spent by this transaction.
    ...
    /** Amount of cachecoins coming in to this transaction
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // cachecoin: get transaction coin age
...
public:
    ...
    // cachecoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // cachecoin: entropy bit for stake modifier if chosen by modifier
...
// DIFF: protocol switch not needed as soon as newer version can be used
    ...
    // cachecoin: two types of block: proof-of-work or proof-of-stake
    ...
    // cachecoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // cachecoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // cachecoin: trust score of block chain
    int nHeight;

    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // cachecoin: block index flags
    enum
    {
