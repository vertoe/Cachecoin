...
#else
    const char* pszModule = "cachecoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cachecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No cachecoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override cachecoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cachecoind.pid"));
